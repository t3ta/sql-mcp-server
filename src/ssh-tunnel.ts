...
export function connectSshTunnel(): Promise<void> {
  return new Promise((resolve, reject) => {
    if (!sshTunnelConfig) {
      ...
      console.error('SSH tunnel configuration not found, skipping tunnel connection.');
      resolve();
      return;
    }

    console.error('Attempting to establish SSH tunnel...');
    tunnel(sshTunnelConfig as TunnelConfig, (error: unknown, server: unknown) => {
      if (error) {
        ...
        console.error('SSH Tunnel established successfully.');
        resolve();
      }
    });
  });
...
export function closeSshTunnel(): Promise<void> {
  return new Promise((resolve, reject) => {
    if (sshServerInstance) {
      console.error('Closing SSH tunnel...');
      ...
      sshServerInstance.close((err: Error | null) => {
        if (err) {
          ...
          console.error('SSH tunnel closed successfully.');
      ...
      console.error('SSH tunnel was not active, skipping closure.');
      resolve();
    }
  });
}
